  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message: "All fields are required, including collection center."
      });
    }
    ...
    if (!validCenter) {
      return res
        .status(400)
        .json({ message: "Invalid collection center selected." });
    }
...
exports.getUserWasteRequests = async (req, res) => {
  try {
    ...
    const requests = await WasteRequest.find({ resident: req.user.id })
...
exports.getWasteProgress = async (req, res) => {
  try {
    ...
    if (!wasteRequests || wasteRequests.length === 0) {
      return res
        .status(404)
        .json({ message: "No waste requests found for this user." });
    }
